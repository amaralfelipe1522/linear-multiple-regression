...
# Estimação do modelo
modelo_recrutam = sm.OLS.from_formula(formula = 'avaliacao ~ treinamentos',
                                    data=gestao_rh).fit()

# Obtenção dos outputs
modelo_recrutam.summary()

#%% Regressão Linear Múltipla - Recrutamento

# Estimação do modelo
modelo_treina_1 = sm.OLS.from_formula(formula = 'avaliacao ~ recrutamento_sites_midias + recrutamento_sourcing',
                                    data=gestao_rh).fit()

# Obtenção dos outputs
modelo_treina_1.summary()

# Estimação do modelo removendo a variável não significante
modelo_treina_2 = sm.OLS.from_formula(formula = 'avaliacao ~ recrutamento_sourcing',
                                    data=gestao_rh).fit()

# Obtenção dos outputs
modelo_treina_2.summary()
